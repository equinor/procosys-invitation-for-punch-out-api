...
namespace Equinor.Procosys.CPO.Infrastructure.Repositories
{
    ...
    {
        protected readonly CPOContext _context;
        protected readonly DbSet<TEntity> Set;
        protected readonly IQueryable<TEntity> DefaultQuery;

        protected RepositoryBase(CPOContext context, DbSet<TEntity> set)
            : this(context, set, set)
        {
        }

        protected RepositoryBase(CPOContext context, DbSet<TEntity> set, IQueryable<TEntity> defaultQuery)
        {
            _context = context;
            Set = set;
