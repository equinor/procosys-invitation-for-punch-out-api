...
namespace Equinor.ProCoSys.IPO.Command.Tests.InvitationCommands.UnCompletePunchOut
{
    ...
    public class UnCompletePunchOutCommandValidatorTests
    {
        ...
        [TestMethod]
        public async Task Validate_ShouldFail_WhenPersonTryingToUnCompleteIsNotThePersonWhoCompletedTheIpoOrAnAdmin()
        {
            ...
            Assert.IsTrue(result.Errors[0].ErrorMessage
                .StartsWith("Person trying to uncomplete is not an admin and not the person who completed the IPO!"));
        }
    }
}
