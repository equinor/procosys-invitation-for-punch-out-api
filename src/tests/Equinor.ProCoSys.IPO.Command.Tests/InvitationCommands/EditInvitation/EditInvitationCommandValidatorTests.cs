...
namespace Equinor.ProCoSys.IPO.Command.Tests.InvitationCommands.EditInvitation
{
    ...
    public class EditInvitationCommandValidatorTests
    {
        ...
        private const string Location = "location A";
        ...
        public void Setup_OkState()
        {
            ...
            _invitationValidatorMock.Setup(inv => inv.IpoExistsAsync(Id, default)).Returns(Task.FromResult(true));
            _invitationValidatorMock.Setup(inv => inv.IpoIsInPlannedStage(Id, default)).Returns(Task.FromResult(true));
            ...
            _invitationValidatorMock.Setup(inv => inv.ParticipantWithIdExistsAsync(_participants[0], Id, default)).Returns(Task.FromResult(true));
            _invitationValidatorMock.Setup(inv => inv.ParticipantWithIdExistsAsync(_participants[1], Id, default)).Returns(Task.FromResult(true));
            _invitationValidatorMock.Setup(inv => inv.OnlyRequiredParticipantsHaveLowestSortKeys(_participants)).Returns(true);
            _invitationValidatorMock.Setup(inv => inv.ProjectNameIsNotChangedAsync(ProjectName, Id, default)).Returns(Task.FromResult(true));
        ...
        {
            _invitationValidatorMock.Setup(inv => inv.IpoTitleExistsInProjectOnAnotherIpoAsync(ProjectName, Title, Id, default)).Returns(Task.FromResult(true));
        ...
        {
            _invitationValidatorMock.Setup(inv => inv.ParticipantWithIdExistsAsync(_participants[0], Id, default)).Returns(Task.FromResult(false));
