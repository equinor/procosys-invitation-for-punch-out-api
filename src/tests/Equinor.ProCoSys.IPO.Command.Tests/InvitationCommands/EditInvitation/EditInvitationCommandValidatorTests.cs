...
namespace Equinor.ProCoSys.IPO.Command.Tests.InvitationCommands.EditInvitation
{
    ...
    public class EditInvitationCommandValidatorTests
    {
        ...
        public void Setup_OkState()
        {
            ...
            _invitationValidatorMock.Setup(inv => inv.ParticipantWithIdExistsAsync(_participants[0], default)).Returns(Task.FromResult(true));
            _invitationValidatorMock.Setup(inv => inv.ParticipantWithIdExistsAsync(_participants[1], default)).Returns(Task.FromResult(true));
        ...
        {
            _invitationValidatorMock.Setup(inv => inv.ParticipantWithIdExistsAsync(_participants[0], default)).Returns(Task.FromResult(false));
