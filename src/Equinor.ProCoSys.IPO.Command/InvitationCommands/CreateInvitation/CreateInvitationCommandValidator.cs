    ...
    public class CreateInvitationCommandValidator : AbstractValidator<CreateInvitationCommand>
    {
        public CreateInvitationCommandValidator(IInvitationValidator invitationValidator)
        {
            ...
            RuleFor(command => command)
                .MustAsync((command, token) => IpoTitleMustBeUniqueInProject(command.ProjectName, command.Title, token))
                .WithMessage(command =>
            ...
            async Task<bool> IpoTitleMustBeUniqueInProject(string projectName, string title, CancellationToken token)
