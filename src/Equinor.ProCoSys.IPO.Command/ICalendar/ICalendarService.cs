...
namespace Equinor.ProCoSys.IPO.Command.ICalendar
{
    public class ICalendarService : IICalendarService
    {
        public Message CreateMessage(Invitation invitation, string projectName, Person organizer, string pcsBaseUrl, CreateInvitationCommand request)
        {
            ...
            string subject = InvitationHelper.GenerateMeetingTitle(invitation, projectName, request.Type,
                            request.Type == DisciplineType.DP ? request.McPkgScope : request.CommPkgScope);
            string content = InvitationHelper.GenerateMeetingDescription(invitation, baseUrl, organizer, projectName, true);
        ...
        private String CreateInvitation(Person organizer, string subject, string content, Invitation invitation)
        {
            ...
            {
                string recipientDisplayName = $"{y.FirstName} {y.LastName}";
        ...
        private FileAttachment CreateICalendarInvitationAttachment(string inviteString)
        {
